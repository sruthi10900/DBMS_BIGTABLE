...
public class RowJoin {
	...
	public void createJoinOutPut() throws FileScanException, TupleUtilsException, InvalidRelation, InvalidTypeException, IOException, HFException, HFBufMgrException, HFDiskMgrException, JoinsException, InvalidTupleSizeException, PageNotReadException, PredEvalException, UnknowAttrType, FieldNumberOutOfBoundException, WrongPermat, InvalidSlotNumberException, InvalidInfoSizeException, SpaceNotAvailableException
	{
		FileScan fsLeft = new FileScan("joinTempLeft", attrType, attrSize, (short) 4, 4, null, null);
		hfJoin = new Heapfile("hfJoinResult");
		Map leftMap = fsLeft.get_next();
		
		while(leftMap!=null)
		{
			FileScan fsRight = new FileScan("joinTempRight", attrType, attrSize, (short) 4, 4, null, null);
			Map rightMap = fsRight.get_next();
			
			Long leftValue = Long.parseLong(leftMap.getValue());
			while(rightMap!=null)
			{
				Long rightValue = Long.parseLong(rightMap.getValue());
				if(leftValue.compareTo(rightValue)==0)
				{
					Map joinMap = new Map(leftMap);
					joinMap.setHdr();
					joinMap.setRowLabel(leftMap.getRowLabel()+":"+rightMap.getRowLabel());
					hfJoin.insertRecord(joinMap.getMapByteArray());
				}
				
				rightMap = fsRight.get_next();
			}
			
			leftMap = fsLeft.get_next();
		}
...
//		FileScan joinOutput = new FileScan("hfJoinResult", attrType, attrSize, (short) 4, 4, null, null);
		
//		Map t = joinOutput.get_next();
